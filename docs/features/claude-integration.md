...
수집기로 전송한다. 요청 ID를 기준으로 중복 이벤트를 제거하고, `session.id`는 즉시
SHA-256 익명 키로 변환해 오늘 사용한 고유 세션 수를 집계한다. 원본 세션 ID는
저장하지 않는다. 연동 전에 이미 실행 중이던 프로세스는 설정을 다시 읽지 않으므로
재시작 전 요청은 수집되지 않는다.

상단의 `활성 AI`는 최근 15분 동안 이벤트를 보낸 공급자 수다. Claude는 같은 기간의
고유 세션 수도 함께 계산한다. 기존에 저장된 이벤트에는 익명 세션 키가 없으므로 이
기능 배포 후 새 요청부터 세션 수에 포함된다.
