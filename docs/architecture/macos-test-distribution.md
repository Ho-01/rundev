...
`main`에 앱 소스, Rust 소스, 의존성 또는 빌드 워크플로 변경이 푸시되면
`macOS test build` 워크플로가 자동 실행된다. 문서만 바뀐 푸시에는 실행하지 않는다.
같은 브랜치에 새 푸시가 들어오면 진행 중인 이전 빌드는 취소해 runner 사용량을 줄인다.

`v*` 태그는 별도의 `Release RunDev` 워크플로를 실행한다. 태그와 앱 버전이 일치할
때만 Windows NSIS와 macOS universal DMG를 GitHub Release 자산에 업로드한다. 현재
